...
# 파일 경로 지정
file_path = "C:/Users/user_pc/Desktop/4학년 1학기/오픈소스소프트웨어프로젝트/팀프로젝트/Dodge-game/"

# Load the background image 
background_image = pygame.image.load(file_path+"background.jpg").convert()
pygame.display.set_caption("OSSP - DeepBug - Dodge v0.0.5")
...
def game_loop():
    ...
    # 랭킹구현
    with open(file_path+"highscore.txt", "r") as f :
        highscore = f.read()
        ...
        for index, fireball in enumerate(fireballs):
            ...
            if fireball.rectangle().colliderect(player.rectangle()):
                ...
                if score > int(highscore) :
                    highscore = score
                with open(file_path+"highscore.txt", "w") as f :
